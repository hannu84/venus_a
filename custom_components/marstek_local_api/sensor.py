...
def _venus_a_pv_power(data: dict, input_number: int) -> float | None:
    """Return Venus A PV input power.

    On the tested Venus A, pv1_power is reported with a factor of 10 while
    pv2_power through pv4_power are already watts.
    """
    raw = data.get("pv", {}).get(f"pv{input_number}_power")
    if raw is None:
        return None
    try:
        value = float(raw)
...
def _venus_a_pv_total_power(data: dict) -> float | None:
    """Return sum of Venus A PV input powers."""
    values = [_venus_a_pv_power(data, index) for index in range(1, 5)]
...
# PV sensors for Venus A.
VENUS_A_PV_SENSOR_TYPES: tuple[MarstekSensorEntityDescription, ...] = (
    MarstekSensorEntityDescription(
        key="venus_a_pv_total_power",
        name="PV input total power",
        native_unit_of_measurement=UnitOfPower.WATT,
        device_class=SensorDeviceClass.POWER,
        state_class=SensorStateClass.MEASUREMENT,
        value_fn=_venus_a_pv_total_power,
        category="pv",
    ),
    MarstekSensorEntityDescription(
        key="venus_a_pv1_power",
        name="PV1 input power",
        native_unit_of_measurement=UnitOfPower.WATT,
        device_class=SensorDeviceClass.POWER,
        state_class=SensorStateClass.MEASUREMENT,
        value_fn=lambda data: _venus_a_pv_power(data, 1),
        category="pv",
    ),
    MarstekSensorEntityDescription(
        key="venus_a_pv2_power",
        name="PV2 input power",
        native_unit_of_measurement=UnitOfPower.WATT,
        device_class=SensorDeviceClass.POWER,
        state_class=SensorStateClass.MEASUREMENT,
        value_fn=lambda data: _venus_a_pv_power(data, 2),
        category="pv",
    ),
    MarstekSensorEntityDescription(
        key="venus_a_pv3_power",
        name="PV3 input power",
        native_unit_of_measurement=UnitOfPower.WATT,
        device_class=SensorDeviceClass.POWER,
        state_class=SensorStateClass.MEASUREMENT,
        value_fn=lambda data: _venus_a_pv_power(data, 3),
        category="pv",
    ),
    MarstekSensorEntityDescription(
        key="venus_a_pv4_power",
        name="PV4 input power",
        native_unit_of_measurement=UnitOfPower.WATT,
        device_class=SensorDeviceClass.POWER,
        state_class=SensorStateClass.MEASUREMENT,
        value_fn=lambda data: _venus_a_pv_power(data, 4),
        category="pv",
    ),
)
...
async def async_setup_entry(
    ...
    if isinstance(coordinator, MarstekMultiDeviceCoordinator):
        ...
        for mac in coordinator.get_device_macs():
            ...
            # Add PV sensors. Venus D uses aggregate PV fields; Venus A uses per-input fields.
            ...
            elif _is_venus_a_model(device_coordinator.device_model):
                for description in VENUS_A_PV_SENSOR_TYPES:
                    entities.append(
                        MarstekMultiDeviceSensor(
                            coordinator=coordinator,
                            device_coordinator=device_coordinator,
        ...
        elif _is_venus_a_model(coordinator.device_model):
            for description in VENUS_A_PV_SENSOR_TYPES:
                entities.append(
                    MarstekSensor(
                        coordinator=coordinator,
