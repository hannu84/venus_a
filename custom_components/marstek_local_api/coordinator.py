...
class MarstekMultiDeviceCoordinator(DataUpdateCoordinator):
    ...
    def _calculate_aggregates(self) -> dict[str, Any]:
        ...
        # Energy aggregates
        def _solar_power_from_device(device_data: dict[str, Any]) -> float:
            es_pv_power = device_data.get("es", {}).get("pv_power")
            if es_pv_power not in (None, 0):
                return es_pv_power
            pv_data = device_data.get("pv", {})
            return (
                (pv_data.get("pv1_power", 0) or 0) * 0.1
                + (pv_data.get("pv2_power", 0) or 0)
                + (pv_data.get("pv3_power", 0) or 0)
                + (pv_data.get("pv4_power", 0) or 0)
            )
