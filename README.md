# Marstek Local API - Venus A Fork

This repository is based on [jaapp/ha-marstek-local-api](https://github.com/jaapp/ha-marstek-local-api).

Venus A additions in this fork:

- Query `PV.GetStatus` for Venus A devices as well.
- Add PV1-PV4 input power sensors and PV input total power.
- Apply the observed `0.1` factor only to PV1 power.
- Use PV1-PV4 as a solar-power fallback when `ES.GetStatus.pv_power` reports `0`.
